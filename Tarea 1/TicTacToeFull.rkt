...
(define (cicloDeJuego matriz jugadorActual)
  (if (equal? (matrizLlena matriz)  #t)
      (print "Matriz llena")
      (if (= jugadorActual 1)
          (cicloDeJuego (caddr(ejecutarMovimientoJugador matriz (string->number (read-line)) (string->number (read-line)) (length matriz) (length (car matriz))))       
                        2
          ) 
          (cicloDeJuego (caddr (colocarFichaMaquina matriz 2))
                        1
          )
      )
  )     
...
(define (funcionDeViabilidad matriz candidatosParaColocar fichasJugador fichasMaquina)
  ...
  (if (not (null? (revisarGaneHorizontal fichasMaquina fichasMaquina (length (car matriz)) '() candidatosParaColocar)))
      (printPrueba(append (list (append (list (append (list (car (revisarGaneHorizontal fichasMaquina fichasMaquina (length (car matriz)) '() candidatosParaColocar)))
                                                      (list 1)
                                  )
                            )
      ...
      (if (not (null? (revisarGaneVertical fichasMaquina fichasMaquina (length matriz) '() candidatosParaColocar)))
          (printPrueba(append (list (append (list (append (list 1)
          ...
          (if (not (null? (revisarGaneDiagonal matriz (diagonalesValidas matriz) '() 2 "Logica")))
              (printPrueba(append (list (append (list (car (revisarGaneDiagonal matriz (diagonalesValidas matriz) '() 2 "Interfaz")))

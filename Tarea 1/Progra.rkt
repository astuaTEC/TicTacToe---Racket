...
(define (cicloDeJuego matriz jugadorActual)
  (if (equal? (hayGanador matriz '() '() jugadorActual)  #t)
      ...
      (if (= jugadorActual 1)
          ...
          (cicloDeJuego (colocarFichaMaquina matriz 2)
                        1
          )
      )
  )     
  ...
  (if (null? diagonales)
      ...
      (if (null? fichaGanar)
          (revisarGaneDiagonal matriz
                               (cdr diagonales)
                               (fichaParaGanarDiagonal matriz (car diagonales) (car diagonales) (caar diagonales) usuario 0)
                               usuario)
          fichaGanar)
      )
)
...
#|
(define (fichaParaGanarDiagonal matriz diagonalActual diagonalActualAuxiliar posicionActual usuario cantidadDiagonales)
  (if (null? diagonalActual)
      cantidadDiagonales
      (if (= (encontrarFichaPorPosicion matriz (caar nPosicion mActual)
|#
...
(define (funcionDeViabilidad matriz candidatosParaColocar fichasJugador fichasMaquina)
  ...
  ;Maquina revisa que no pueda perder
  (if (not (null? (revisarGaneHorizontal fichasJugador fichasJugador (length (car matriz)) '() candidatosParaColocar)))
      (colocarFichaJugador (car (revisarGaneHorizontal fichasJugador fichasJugador (length (car matriz)) '() candidatosParaColocar))
      ...
      (if (not (null? (revisarGaneVertical fichasJugador fichasJugador (length matriz) '() candidatosParaColocar)))
          (colocarFichaJugador (car (revisarGaneVertical fichasJugador fichasJugador (length (car matriz)) '() candidatosParaColocar))
                               (cadr (revisarGaneVertical fichasJugador fichasJugador (length (car matriz)) '() candidatosParaColocar))
                               1
                               (length matriz)
                               matriz
                               '()
          ...
          (if (not (null? (revisarGaneDiagonal matriz (diagonalesValidas matriz) '() 1)))
              (colocarFichaJugador (car (revisarGaneDiagonal matriz (diagonalesValidas matriz) '() 1))
